...
- **저장은 회차 한 줄만 건드립니다.** 두 사람이 같은 회차를 동시에 고치면
  조용히 덮어쓰지 않고, 누가 언제 저장했는지 알려 준 뒤 사람이 정하게 합니다.
- **입력자는 화면에서 직접 적는 이름입니다.** 계정을 두지 않기로 했으므로
  "김OO 이 적었다"가 사실인지는 보증되지 않습니다. 수정시각은 서버가 정확히
  기록합니다. 나중에 사내 계정을 붙이면 `backend/app/api/entries.py` 의
  `entered_by()` 와 `frontend/src/lib/whoami.ts` 만 바꾸면 됩니다.
