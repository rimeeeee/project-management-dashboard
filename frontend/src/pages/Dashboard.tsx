...
export default function Dashboard({
  ...
  const [펼친월, set펼친월] = useState<Set<string>>(() => new Set());
  ...
  // 계정과목은 사업당 하나이고, 그 아래 세목 편성액을 모두 더한 값이
  // 계정과목에 잡힌 총 편성액입니다. 총 사업비와 다를 수 있어 따로 냅니다.
  const 계정과목편성액 = p.categories.reduce((sum, c) => sum + c.allocated, 0);

  // 사업 수정에서 등록한 세목은 집행액이 0원이어도 월 상세에 냅니다.
  // 예전에 사용했지만 현재는 삭제된 세목도 실제 집행 내역이 남아 있으면 뒤에 붙입니다.
  const 월별세목 = [
    ...p.categories.map((c) => c.name),
    ...p.monthly.rows.map((r) => r.cat).filter((name) => !p.categories.some((c) => c.name === name)),
  ];
      ...
      <div className={"dash-wrap" + (inputOpen ? " open" : "")} id="dashWrap">
        <div className="dash-grid">
          ...
          <div className="card">
            ...
            {/* 계정과목 — 세목 편성액의 합과 실제 집행액을 함께 보여 줍니다.
                총 사업비와 세목 편성 합계가 다를 수 있으므로 둘을 섞지 않습니다. */}
            {p.account ? (
              <div className="acct-line">
                ...
                <span className="acct-amount">
                  <span className="k">총 편성</span>
                  <span className="v">{계정과목편성액 > 0 ? fmtMoney(계정과목편성액) : "미입력"}</span>
                </span>
                <span className="acct-amount">
                  <span className="k">집행</span>
                  <span className="v">{fmtMoney(p.spent)}</span>
                </span>
              </div>
            ) : (
          ...
          {/* 월별 집행 — 월·계정과목·집행액만 먼저 보여 주고, 필요한 달만
              펼쳐 세목/비목별 금액을 확인합니다. */}
          {p.monthly.months.length > 0 && (
            <div className="card span2 mo-card">
              <h2>월별 집행
                <span className="hint">
                  지출일 기준 · 전체 누계 {fmtMoney(p.monthly.grand)}
                </span>
              </h2>
              <div className="mo-months">
                {p.monthly.months.map((m) => {
                  ...
                  return (
                    <div key={m} className={"mo-month" + (열림 ? " open" : "")}>
                      <button type="button" className="mo-summary" aria-expanded={열림}
                              aria-controls={`mo-detail-${m}`}
                              onClick={() => set펼친월((before) => {
                                const next = new Set(before);
                                if (next.has(m)) next.delete(m); else next.add(m);
                                return next;
                              })}>
                        <span className="mo-date">{Number(m.slice(0, 4))}년 {Number(m.slice(5))}월</span>
                        <span className="mo-account">{p.account || "미지정"}</span>
                        <span className="mo-amount">{fmtWon(합)}원</span>
                        <span className="mo-chevron" aria-hidden="true">⌄</span>
                      </button>
                      {열림 && (
                        <div className="mo-detail" id={`mo-detail-${m}`}>
                          {월별세목.length ? 월별세목.map((name) => {
                            const row = p.monthly.rows.find((r) => r.cat === name);
                            const amount = row?.byMonth[m] ?? 0;
                            return (
                              <div className="mo-detail-row" key={name}>
                                <span>{name}</span><strong>{fmtWon(amount)}원</strong>
                              </div>
                            );
                          }) : <div className="mo-detail-empty">등록된 항목이 없습니다.</div>}
                        </div>
                      )}
                    </div>
                  );
              ...
              <div className="mo-grand">
                <span>전체 누계</span><strong>{fmtWon(p.monthly.grand)}원</strong>
              </div>
            </div>
          )}
